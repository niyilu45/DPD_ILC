...
- [DPD-ILC 常见问题](doc/FAQ.md)：低功率小信号逆响应、IRR用途/原理/计算与场景、普通/增广GMP的镜像补偿边界、仪表整段采集下的逐样点DPD训练、史密斯圆图、FFT/Welch功率谱选择，以及DPD迭代带外增长和帧头高PAPR训练变差的原因、判据与A/B排查流程。
...
doc/FAQ.md              小信号逆响应、IRR、增广/分段GMP、逐样点训练、史密斯圆图、FFT/Welch、迭代带外增长和帧头高PAPR训练FAQ
...
浮点和定点模式的数组形状与 `numpy.complex128` 类型相同，但数值含义不同。定点模式的实部和虚部是整数码：14 位范围为 `-8192…8191`，16 位范围为 `-32768…32767`。$F$ 是 `fullScaleAmplitude`：输入DAC仍默认 $F=1$；`PaModel`、`MimoPaModel` 和 `Channel` 固定输出默认 $F=2$，即比单位归一化幅度多6.02 dB分量观测余量。模块收到码值后按相应 $F$ 解码，内部 FFT、PA 非线性、同步、ILC 和指标计算始终使用浮点。由于 $F$ 可以配置，这里统一称为 **scaled full-scale**，不把任意标尺笼统叫作Q1.15或Q2.14。

```mermaid
flowchart LR
    input["外部 complex128<br/>I/Q为整数码"] --> quantizer["码值舍入与饱和"]
    quantizer --> floating["乘 F/2^(W-1)<br/>内部浮点"]
    floating --> algorithm["模块内部浮点算法"]
    algorithm --> outputQuantizer["乘 2^(W-1)/F<br/>舍入与饱和"]
...
- `GMPPA.Process` 使用 `DelaySignal` 构造主项、滞后包络项和超前包络项；未提供系数时，`DefaultGmpCoefficients` 创建在 $0\leq|x|\leq2$ 内单调的参考稳态曲线，普通 `GMPConfig` 默认以 `nonlinearScale=0.135` 缩放三阶及以上参考项，再按各阶有效稳态系数比例生成较小的非线性延迟与交叉项，并从零延迟主项中抵消其总和；一阶线性尾项仍是独立的小信号FIR。当三张系数表全部省略时，内置普通GMP叠加归一化的12抽头三阶长包络记忆，其稳态总和为零，但记忆长度超过默认DPD的 `crossMemoryDepth=2`。非默认阶次组合会在需要时进一步缩小非线性项以保持同一区间单调，未知高阶默认值为0。
...
| `GetLastPaOutput()` | 无 | 返回闭环最后一次PA实测输出，无需重复激励PA；定点码使用绑定plant的输出标尺，独立Analysis/TwoToneAnalysis必须显式接收同一标尺。 |
...
当前构造函数签名为
`PaModel(modelName=None, rappConfig=None, wienerConfig=None, gmpConfig=None, piecewiseGmpConfig=None, dohertyConfig=None, thermalConfig=None, parameters=None, width=None, outputFullScaleAmplitude=None, **parameterOverrides)`。
模型参数既可以直接传入，也可以放入 `parameters` 映射；直接参数优先级更高。

| 参数 | 类型或可选值 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `parameters` | `Mapping` | `None` | 调用方只传需要修改的键；缺少的键由 `PaModel` 构造函数内部的不可变默认参数补齐。 |
| `width` | 非负整数 | `16` | PA输入、输出I/Q接口位宽；`0`为浮点，正数返回I/Q整数码，容器仍为 `complex128`。输入DAC标尺保持1.0。 |
| `outputFullScaleAmplitude` | 正有限实数 | `2.0` | 固定点PA输出正满码所代表的I或Q物理分量幅度；默认2.0提供6.02 dB观测余量，不改变输入DAC标尺或 `maximumOutputPowerDbm` 的单位RMS功率锚点。 |
...
`MimoPaModel(parameters=None, width=None, outputFullScaleAmplitude=None, **parameterOverrides)` 在构造函数内部使用 `ChainMap` 管理以下参数：

| 参数 | 默认值 | 说明 |
| --- | --- | --- |
| `numTransmitChains` | `1` | 独立 PA 数，范围 1–16。 |
| `width` | `16` | 整个输入矩阵和每路输出的I/Q整数码位宽；内部每路PA解码后仍用浮点计算。 |
| `outputFullScaleAmplitude` | `2.0` | 全部固定点PA输出列的scaled full-scale分量标尺；输入矩阵标尺仍为1.0。 |
...
```python
Channel(paModel=None, parameters=None, width=None, outputFullScaleAmplitude=None, **parameterOverrides)
```
...
| `channelFractionalDelayFilterOrder` | `23` | 公共分数时延滤波器阶数；`1`保留旧两抽头线性插值，奇数 `3, 5, ...` 使用 `order+1` 抽头的归一化Lanczos核。整数 `channelDly` 精确旁路该滤波器。 |
...
`channelDly=D_total` 分解为 `D=floor(D_total)` 与 `mu=D_total-D`。`mu=0` 时直接执行精确整数移位。`mu!=0` 时先在原记录坐标上执行分数时延FIR，再前补 `D` 个零并截去同长尾部。默认阶数 `P=23` 对应 `L=(P+1)/2=12`、24抽头归一化Lanczos核，其lag支撑为 `[-11, 12]`；因此离线计算会读取当前位置之后最多11个输入样点。记录外部按0处理，滤波器只对整组抽头归一化一次，不会在边界按剩余抽头重新归一化。名义物理时间仍为 `D_total/sampleRateHz`。示例：
...
两路都包含 `3.25 sample` 公共时延；fb模式的第二路还叠加FB专用时延。`channelDly` 是非负物理传播时延，默认由可配的高阶Lanczos核生成；`fbFractionalDelaySamples` 则是可带符号的接收采样偏差，目前仍使用一阶线性插值。两级串联时名义group delay相加，但实际频响为两级频响相乘，不等价于把总小数部分只插值一次。forward模式仍直接复制延迟后的 `chOut`，所以两项完全相同。功率闭环测量公共时延之前的干净PA输出，不会把同长零边界造成的表观记录功率变化反馈到PA drive。

生成端和补偿端的“阶数”不是同一个参数：

| 位置 | 当前选择量 | 实际规模 | 能解决的问题 |
| --- | --- | ---: | --- |
| Channel公共 `channelDly` | `channelFractionalDelayFilterOrder=P`，默认23 | `P+1` taps，默认24 taps；`P=1`为两抽头旧模式 | 生成整数加分数传播时延；阶数决定注入端带内近似误差、前视和边界guard |
| SigProc接收补偿 | `interpolationHalfLength=L`，默认12 | $2L$ taps，默认24 taps/23阶等效FIR | 更准确地重采样已收到的波形；不能恢复注入端、硬件通道或边界已经造成的幅频失真 |

注入端与接收端必须分开验收：先用独立长sinc、零扩展FFT或硬件时延作真值，在已知时延下分别扫描Channel的 `P` 和接收端的 `L`，再开启自动估计。对每一端都应同时要求绝对EVM余量和阶数加倍收敛；已知时延通过而自动路径停在平台时，限制来自时延估计而不是补偿滤波器阶数。普通筛选可留10 dB余量；若要求该模块使最终EVM恶化不超过0.1 dB，应留约17 dB。默认23阶Channel需在评价区前后至少保留其真实支撑要求的guard；流式实现还需缓存跨块历史及11个前视样点，或显式增加11点流水延迟使其因果化。完整公式、guard计算和故障判据见 [Channel §1.1](doc/Channel.md#11-chout前向仪表采样与公共传播时延) 与 [SigProc §7](doc/SigProc.md#7-重采样与分数时延补偿)。
...
| 公共传播时延 | `channelDly=0.0` 为理想基线；功能验证可用 `3.25 sample` | PA后耦合之后共同延迟chOut/fbOut；默认 `channelFractionalDelayFilterOrder=23` 先执行24抽头归一化Lanczos分数滤波，再对整数部分前补零/尾截断。阶数 `1` 才是旧两抽头模式。物理秒数为 `channelDly/sampleRateHz`，不进入PA功率闭环。 |
