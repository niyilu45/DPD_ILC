"""Generate IEEE 802.11be EHT waveforms for PA and DPD simulation.

The generator creates a single-user, full-band, non-OFDMA EHT-MU complex
baseband packet. It follows the EHT field order, 78.125 kHz data subcarrier
spacing, full-band RU sizes, pilot/data tone counts, guard intervals, and the
MCS 0 through 13 modulation and code-rate table.
...
is an EHT PHY stimulus, not a bit-exact protocol conformance implementation.
"""
...
class MCSInfo:
    """Describe the modulation and nominal coding parameters of one EHT MCS."""
...
# EHT MCS 0 through 13 cover BPSK through 4096-QAM. Keeping the table in one
# place allows callers and tests to inspect every supported operating mode.
mcsTable: Mapping[int, MCSInfo] = {
...
# The 78.125 kHz EHT subcarrier spacing gives these nominal FFT lengths.
...
@dataclass(frozen=True)
class EHTConfig:
    """Store the user-selectable EHT waveform configuration."""
    ...
    def Validate(self) -> None:
        ...
        if self.mcs not in mcsTable:
            raise ValueError("EHT MCS must be an integer from 0 through 13")
...
class EHTWaveform:
    ...
    formatName: str = "EHT-MU non-OFDMA single-user"


def _ActiveTones(bandwidthMhz: int) -> np.ndarray:
    """Return centered subcarrier indices for a full-band EHT resource unit."""
    ...
    if bandwidthMhz == 20:
        ...
        # centers are separated by 1024 EHT subcarrier spacings.
    ...
    if toneIndices.size != _ruToneCount[bandwidthMhz]:
        raise RuntimeError("internal EHT RU tone construction error")
    return toneIndices.astype(np.int32)


def _PilotTones(activeTones: np.ndarray, bandwidthMhz: int) -> np.ndarray:
    """Build a symmetric full-band pilot pattern with the EHT pilot count."""
    ...
    # requirement of sixteen pilot tones across the full 80 MHz allocation.
...
def _TrainingField(
    ...
    """Generate deterministic 4 us pre-EHT training or signaling symbols."""
...
def GenerateEhtWaveform(config: EHTConfig = EHTConfig()) -> EHTWaveform:
    """Generate one EHT packet plus all metadata needed for later analysis.

    Args:
        config: EHT bandwidth, MCS, guard interval, packet length, and seed.

    Returns:
        An ``EHTWaveform`` containing unit-RMS samples, field boundaries,
        subcarrier allocations, and the transmitted data constellation.
    """

    config.Validate()
    randomGenerator = np.random.default_rng(config.seed)
    mcsInfo = mcsTable[config.mcs]
    ...
    if pilotSubcarriers.size != _pilotToneCount[config.bandwidthMhz]:
        raise RuntimeError("internal EHT pilot tone construction error")
    ...
    # The EHT-MU preamble begins with legacy-compatible fields, followed by
    # universal/EHT signaling and the EHT-specific training fields.
    ...
    # EHT-STF is a 4 us field. A legacy-rate OFDM construction preserves that
    # duration while providing a repeatable wideband training stimulus.
    AppendField(
        "EHT-STF",
        _TrainingField(
            1,
    ...
    # A 1.6 us GI is paired with 2x EHT-LTF compression (6.4 us useful
    # duration). The 0.8 us and 3.2 us GIs use a 4x EHT-LTF (12.8 us useful
    # duration). For 2x compression only every second frequency tone is sent.
    if config.guardIntervalUs == 1.6:
        ehtLtfFftLength = fftLength // 2
        evenToneMask = np.mod(activeSubcarriers, 2) == 0
        ehtLtfSubcarriers = activeSubcarriers[evenToneMask] // 2
    else:
        ehtLtfFftLength = fftLength
        ehtLtfSubcarriers = activeSubcarriers
    ehtLtfCpLength = int(
        round(config.guardIntervalUs * 1e-6 * sampleRateHz)
    )
    ehtLtfValues = _PilotSequence(ehtLtfSubcarriers.size, randomGenerator)
    AppendField(
        "EHT-LTF",
        _OfdmSymbol(
            ehtLtfFftLength,
    ...
    AppendField("EHT-Data", dataSamples)
    if fieldSlices["EHT-Data"].start != dataStart:
        raise RuntimeError("internal EHT data field offset error")
