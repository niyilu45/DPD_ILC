...
旧的内置PA与DPD采用了非常接近的短记忆GMP结构。无噪声时，若每个功率点
分别训练一个逆模型，可重复非线性会被近乎完全抵消，随后只剩随信号同比缩放的
...
上面的 `FitSegments` 把多个功率片段共同求解为**一组**系数。验收时必须冻结
这组系数，在每个功率上使用未参与训练的帧，并让各对照方案把完整DPD+PA串联
重新闭环到该点相同的实测目标输出功率。功率闭环只允许调整驱动量，不能在扫
功率过程中再次更新DPD系数。

如果在1、16、20 dBm各自重新调用 `Fit`，得到的是三个局部逆模型。它们都达到
相近的最优EVM在数学上可以正常，但这项试验测量的是“每个功率都可重新训练”而
不是“一套系数跨功率鲁棒”。需要这种能力时，应建立显式的功率索引系数库，按
实测输出功率、驱动状态及必要的温度状态选取或插值系数；瞬时包络的
low/middle/high区域不能代替平均功率工作点索引。
...
这里的“重新闭环”是让各对照方案在每个功率点都达到该点相同的实测目标dBm，
不是重新训练DPD。做多功率鲁棒性报告时，同一阶段的DPD系数从第一个功率点到
最后一个功率点必须保持不变。ILC逐波形迭代可作为生成标签和估计可达到上限的
oracle，但最终结论必须来自独立验证帧上的有限记忆DPD；否则ILC对本帧确定性
残差的记忆会掩盖第6.1节刻意引入的长记忆结构失配。
