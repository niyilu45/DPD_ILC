...
新的内置普通GMP PA和内置分段GMP PA默认增加了一条零稳态和的长包络三阶
支路：
...
其中默认 $L=12$、$\rho=0.82$、$c=0.008-0.0036j$。因为
$\sum_l h_l=1$，恒包络进入稳态后括号内严格为零，因而不会重复改变原来的
静态AM-AM/AM-PM；包络上升、下降和OFDM峰值附近则会产生三阶、功率相关且
持续12个样点的记忆瞬态。该失配完全确定，不依赖加噪声、随机扰动或人为提前
停止训练。DPD仍保留默认较短的 `memoryDepth=3`、
`crossMemoryDepth=2`，所以有限记忆DPD不能再精确复制并求逆内置PA的全部动态。

这条支路只加在由库自动生成的系数上。普通GMP或分段PA的任一区域只要显式
提供 `mainCoefficients`/`laggingCoefficients`/`leadingCoefficients` 中任意
一张表，工程就不会在该区域暗中叠加长记忆支路。仅提供 `regionConfigs` 但让
某个区域的三张系数表都保持 `None` 时，该区域仍会按自己的
`longEnvelopeMemory*` 参数生成默认长支路。
...
## Q11：为什么默认GMP在16位20 dBm时会测到约 -24 dB，而浮点本征应约 -41 dB？
...
| 16 dBm | 约 -46.9 dB | 约 -46.8 dB |
| 20 dBm | 约 -40.9 dB | 约 -40.9 dB |
...
量程边界复测表明，默认标尺2.0在20 dBm没有rail并保持约 -40.9 dB EVM，是该工作点量化精度与峰值余量的折中；接近额定上限时它可能出现少量rail。把plant及Analysis标尺同时显式设为4.0后，25 dBm测试实测25.098 dBm、EVM约 -33.82 dB，I/Q rail计数为0。
...
assert abs(metrics["evmDb"] - (-41.0)) <= 1.5
```
