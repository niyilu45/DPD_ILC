...
| GMP | IM3间隔变化0.047 dB、最大不对称0.022 dB；默认零和记忆残差只产生弱频率相关记忆 | 从1/3/5阶、主记忆深度3、交叉深度1的浅GMP开始；不要仅因模型名为GMP就直接扩到深度5至7 | 各间隔保持相同实际输出功率，用未参与拟合的中间间隔验证；只有残余间隔变化或侧带差稳定超过测量重复性时才增加lagging/leading深度，且IM5/IM7不能恶化超过1 dB |
...
关键原则是先由“间隔变化”和“侧带不对称”证明交叉记忆项确有必要，再增加模型自由度。当前默认GMP的0.047 dB与0.022 dB都很小，能证明实现中存在动态项，但不足以支持“必须使用深交叉记忆”的结论。否则GMP矩阵会包含大量弱可观测列，使ILC标签拟合和直接学习都更容易过拟合。
...
| GMP | 动态AM-AM为0.011 dB、AM-PM为0.034度，默认20 dBm下只有弱动态迟滞 | 保留浅lagging/leading支路用于捕获已知动态残差；不建议在没有验证收益时加入深交叉记忆或递归状态 | 训练集按幅度箱和包络方向均衡抽样；若浅模型的独立验证已低于0.10 dB和1度，就停止增加记忆自由度 |
...
| GMP | IM3/IM5/IM7为-50.16/-87.23/-129.55 dBc；默认 `nonlinearScale=0.135`，20 dBm仍是温和、单调的Rapp型压缩 | 从正则化1/3/5阶、主深度3、交叉深度1的浅GMP开始，并保留输入峰值投影；只有标签或独立射频验证表明欠拟合时再加入7阶和更深记忆 | 可在12至20 dBm建立局部逆，再用独立帧按相同输出功率验证；当前12 dBm基准实测IM3改善5.21 dB，不能把未实测的20 dBm改善写成既定结论 |
...
**图4说明：**左上为IM3，右上为IM5和IM7；下方为动态AM-AM和AM-PM迟滞。Rapp的互调随压缩增强，但AM-PM始终为0；Wiener在接近25 dBm时压缩迅速增强；GMP的IM3从10至25 dBm单调接近基波，而按每阶稳态系数比例缩放的动态项使AM-AM和AM-PM迟滞在整个扫描内保持较弱；Doherty在Peaking开启前与Wiener一致，开启后形成独立但平滑的功率轨迹，其IM3也随功率单调接近基波，不过IM5和IM7会因支路复数合成呈现不同的阶次趋势。Rapp在25 dBm出现的非零AM-AM“迟滞”来自固定宽度幅度分箱内的曲线陡峭度和有限样点分布，不是模型获得了历史状态。
...
| GMP | 五个扫描点均未越过-30 dBc强失真门限；25.10 dBm时IM3仍为-40.98 dBc、动态AM-PM为0.033度，且IM3功率趋势单调 | 10至25 dBm可先验证一个浅GMP系数集；只有跨功率独立验证显示单一系数不足时，才使用按实测输出功率和包络RMS索引的系数库并约束相邻系数平滑 | 先在12或20 dBm建立稳定模型，再逐级验证23和25 dBm；只有跨功率独立EVM或ACLR确实退化时才引入多功率模型，不能由PA名称直接推定必须分库 |
...
| GMP | 2.050 | 0.201 | 0.080 | 0.567 | 0.047 | 0.022 | 0.011 | 0.034 |
...
| GMP | 20.089 | -50.165 | -87.226 | -129.546 |
...
| GMP | 10 | 10.081 | -69.907 | -127.354 | -186.817 | 0.012 | 0.034 |
| GMP | 20 | 20.089 | -50.166 | -87.228 | -129.549 | 0.011 | 0.034 |
| GMP | 25 | 25.099 | -40.984 | -66.974 | -98.338 | 0.010 | 0.033 |
...
- GMP默认配置以 `nonlinearScale=0.135` 缩放三阶及以上的完整参考稳态系数，再把每一阶目标作为所有支路的系数和，并叠加较小的零和主记忆、滞后和超前残差。因此连续高幅平台的稳态响应不会随记忆深度重复压缩；间隔依赖、侧带不对称和动态迟滞都保持为弱但可测的非零量。只有显式配置 `nonlinearScale=1.0` 才选择 full-reference 压力曲线。
...
- 默认GMP在20.09 dBm处仍为温和非线性，IM3为-50.17 dBc；到25.10 dBm时IM3也仍为-40.98 dBc，五个默认扫描点均未进入本基准的-30 dBc强失真区。该结果只说明 `nonlinearScale=0.135` 默认系数的行为，不表示GMP结构天然优于或劣于Wiener、Doherty。用实测PA拟合系数或显式 full-reference 系数替换默认值后，应重新生成全部表格。
...
12 dBm是当前基准的标称训练点，15 dBm是同一系数结构的较高功率压力点。第6节的新功率扫描显示默认GMP到20.09 dBm时IM3仍为-50.17 dBc，25.10 dBm时也为-40.98 dBc；因此12和15 dBm都处于单调、可稳定求逆的温和失真区域。二者的比较用于量化功率回退带来的误差裕量，不表示15 dBm已经深压缩。
...
|---|---:|---:|---:|---:|---:|
| PA baseline stress | 15.085 | -48.871 | 33.031 | -59.990 | — |
| PA baseline nominal | 12.083 | -50.524 | 33.038 | -65.947 | — |
| Basic DPD-GMP stress | 15.083 | -54.038 | 33.065 | -65.691 | -68.094 |
| Basic DPD-GMP nominal | 12.082 | -56.273 | 33.076 | -71.152 | -68.094 |
| Memory-expanded DPD-GMP | 12.082 | -56.678 | 33.071 | -72.701 | -71.317 |
| Peak-weighted DPD-GMP | 12.082 | -56.678 | 33.078 | -73.193 | -70.589 |
| Regularized DPD-GMP | 12.082 | -56.550 | 33.063 | -72.831 | -70.275 |
| Multi-power DPD-GMP | 12.082 | -56.246 | 33.063 | -70.782 | -69.243 |
...
|---|---:|---:|---:|---|
| Wi-Fi EVM | -50.524 dB | -56.273 dB | 5.749 dB | 是，EVM更负 |
| 双音IM3 | -65.947 dBc | -71.152 dBc | 5.205 dB | 是，IM3更负 |
| ACLR | 33.038 dB | 33.076 dB | 0.037 dB | 是，ACLR更高 |

Wi-Fi训练的系数不仅在独立seed的Wi-Fi帧上改善EVM与ACLR，而且在没有双音重训练的情况下仍改善IM3，说明模型学到的是可迁移的PA局部逆，而不只是记住一组训练样点。

### 12.4 改进二：较高功率压力点执行输出回退

**PA依据：**功率扫描中15.08 dBm的IM3为-59.97 dBc，仍未进入深压缩，但相较12 dBm有更大的非线性误差。降低输出功率会同时增加局部斜率裕量和峰值余量，因此应能进一步降低EVM。
...
|---|---:|---:|---:|---|
| Wi-Fi EVM | -54.038 dB | -56.273 dB | 2.235 dB | 是 |
| ACLR | 33.065 dB | 33.076 dB | 0.010 dB | 是 |

在15 dBm处，基础DPD仍把独立验证帧上的同功率PA baseline EVM从-48.871 dB改善到-54.038 dB，改善5.167 dB，说明该点可由当前受限结构稳定补偿。回到12 dBm后的额外改善证明的是功率回退收益，而不是15 dBm不可逆。

### 12.5 改进三：扩展非线性和交叉记忆

**PA依据：**默认GMP的IM3间隔变化只有0.047 dB、最大上下侧不对称0.022 dB、动态AM-PM迟滞0.034度，属于弱记忆。扩展结构在这里是受控的模型容量实验，而不是由强迟滞强制得出的结构需求。
...
|---|---:|---:|---:|---|
| 12 dBm标签NMSE | -68.094 dB | -71.317 dB | 3.223 dB | 是，表达能力提高 |
| Wi-Fi EVM | -56.273 dB | -56.678 dB | 0.405 dB | 小幅改善 |

本项预期是“更准确描述ILC标签”，不是保证射频指标同比例下降。标签NMSE改善3.223 dB，而独立帧EVM只改善0.405 dB，符合默认记忆较弱的测量事实，也说明继续增加结构的边际射频收益已经很小。
...
|---|---:|---:|---:|---|
| 峰值加权标签NMSE | -72.992 dB | -73.868 dB | 0.876 dB | 是 |
| 普通标签NMSE | -71.317 dB | -70.589 dB | -0.728 dB | 允许的权衡 |
...
| Wi-Fi EVM | -56.678 dB | -56.550 dB | -0.128 dB | 允许的小幅代价 |
...
|---|---:|---:|---:|---|
| 10/12/14 dBm最差标签NMSE | -62.510 dB | -64.806 dB | 2.296 dB | 是 |
| 10/12/14 dBm最差ACLR | 33.062199 dB | 33.062091 dB | -0.000108 dB | 是，退化未超过0.10 dB容限 |
| 10/12/14 dBm最差EVM | -55.453 dB | -55.255 dB | -0.199 dB | 跨功率EVM折中 |
| 12 dBm EVM | -56.550 dB | -56.246 dB | -0.304 dB | 单点性能折中 |

多功率模型改善了预先声明的最差标签NMSE；独立验证帧的最差ACLR只下降0.000108 dB，满足“不退化超过0.10 dB”的预设容限。与此同时最差EVM和12 dBm单点EVM都略有退化。这不是“全面更好”，而是当前1/2/1权重下的明确折中。这说明下一步有两种选择：
