...
| GMP | 增益起伏0.202 dB、群时延0.080 ns、相位曲率0.567度；存在可测但较浅的一阶动态记忆 | 先使用3至5抽头复FIR，或保留GMP的一阶主记忆项；只有独立验证仍有系统性残差时才扩到7抽头 | 用多音或宽带波形覆盖带边，采用岭正则联合求解线性项和非线性项；必须保留未参与拟合的带边频点做验证 |
...
| PA | 测试结论 | 针对性DPD结构与初始参数 | 训练和验收建议 |
|---|---|---|---|
| Rapp | IM3间隔变化0.052 dB、最大上下侧差约0 dB；微小变化来自有限记录投影而不是模型状态 | 使用严格无记忆LUT或主记忆深度1、交叉深度0的静态多项式 | 多个间隔只用于验证“不随间隔变化”，不应把所有间隔重复用于估计延迟项；验收要求IM3间隔变化和侧带不对称均小于0.10 dB |
| Wiener | IM3间隔变化0.007 dB、最大不对称0.001 dB，非线性部分近似无记忆 | 使用奇数阶1/3/5/7、主记忆深度3的Memory Polynomial；初始不加入lagging/leading包络交叉项 | 用最小、4 MHz和最大间隔训练，并用其余间隔验证。若剩余IM3间隔变化仍小于0.5 dB，不应为了模型复杂度加入GMP交叉项 |
| GMP | IM3间隔变化11.084 dB、最大不对称11.740 dB；12抽头零稳态和长包络支路产生显著且相位主导的频率相关记忆 | 从1/3/5阶、主记忆深度3、交叉深度2的GMP开始，并把长lagging-envelope支路或等效包络状态作为候选 | 各间隔保持相同实际输出功率，用未参与拟合的中间间隔验证；若短记忆残差仍超过测量重复性，再增加lagging/leading深度，且IM5/IM7不能恶化超过1 dB |
| Doherty | IM3间隔变化0.013 dB、最大不对称0.034 dB；默认支路没有明显额外时延，但Carrier/Peaking合成仍产生弱侧带差 | 使用每个工作区奇数阶1/3/5/7、深度3的浅记忆模型，并在Carrier/Peaking区域之间使用连续平滑门控 | 除间隔扫描外增加Peaking开启附近的功率点；若真实硬件支路存在时延，再按实测结果增加支路专用记忆，不能仅依据本默认模型省略 |

关键原则是先由“间隔变化”和“侧带不对称”证明交叉记忆项确有必要，再增加模型自由度。当前默认GMP的11.084 dB与11.740 dB远高于无记忆数值基线，支持测试更深的包络记忆；但最终深度仍应由独立间隔和Wi-Fi帧的验证收益决定，避免把大量弱可观测列无条件加入回归矩阵。
...
**图2说明：**左上为IM3随双音间隔的变化，右上为上下侧IM3绝对不对称；下方两图分别为动态AM-AM和AM-PM迟滞。Rapp提供接近0的无记忆数值基线；默认GMP的12抽头长包络支路使间隔变化、侧带差和动态迟滞都明显增大。零稳态和只保证恒包络稳态曲线不移动，并不要求宽带包络的动态指标接近0。
...
| PA | 测试结论 | 针对性DPD结构与初始参数 | 训练和验收建议 |
|---|---|---|---|
| Rapp | 20 dBm下动态AM-AM约0.0000003 dB、AM-PM约0度；上升和下降轨迹理论上完全重合 | 只使用当前样点幅度的静态逆，主记忆深度1、交叉深度0，关闭递归或包络状态支路 | 用相同幅度分箱验证上下行轨迹重合。若测得明显迟滞，应先检查Channel、热状态、同步和分箱误差，不能把它归因于基础Rapp方程 |
| Wiener | 动态AM-AM为0.061 dB、AM-PM为0.014度，迟滞很弱 | 保持静态逆加深度2至3的Memory Polynomial；所有延迟非线性项使用较强岭正则 | 对每个幅度箱平衡上升和下降样本。若验证迟滞仍低于0.10 dB和1度，不引入递归状态或大量交叉项 |
| GMP | 动态AM-AM为0.028 dB、AM-PM为0.898度；较小的幅度迟滞与显著相位迟滞表明长包络支路以相位动态为主 | 保留lagging/leading支路并验证12抽头长包络候选；只有独立帧有收益时才采用完整深度或紧凑包络状态 | 训练集按幅度箱和包络方向均衡抽样，同时用未训练间隔检查频域残差；短记忆模型必须用独立帧确认其剩余动态误差 |
| Doherty | 动态AM-AM为0.061 dB、AM-PM为0.015度；默认模型迟滞弱，但Peaking切换可能在真实硬件产生方向相关性 | 使用Carrier-only和Carrier+Peaking两个系数专家，以连续门函数平滑混合；初始每个专家深度3 | 在Peaking门限附近提高样本密度，并约束两套模型在边界处的函数值和一阶斜率连续，避免DPD自身制造迟滞环 |

对于当前默认GMP，20 dBm动态AM-AM仅0.028 dB，但动态AM-PM达到0.898度，同时11.084 dB的IM3间隔变化已经证明存在显著、相位主导的长包络动态；采用深GMP还是紧凑状态模型仍须由独立EVM、间隔扫描和标签NMSE共同决定。对于Doherty，重点仍不是无条件加深记忆，而是防止区域切换不连续。
...
| PA | 测试结论 | 针对性DPD结构与初始参数 | 训练和验收建议 |
|---|---|---|---|
| Rapp | IM3/IM5/IM7为-56.29/-65.92/-83.24 dBc，属于轻度静态压缩且没有AM-PM | 使用单调幅度LUT，或1/3/5/7阶、深度1的静态多项式；不需要共轭或延迟支路 | 对输入幅度分箱均衡采样，在饱和斜率趋近0之前停止求逆；同功率验收要求IM3至少改善10 dB且IM5/IM7不退化超过1 dB |
| Wiener | IM3/IM5/IM7分别为-40.93/-63.57/-85.76 dBc，属于轻度非线性且阶次衰减正常 | 采用1/3/5/7阶、深度3的Memory Polynomial或GMP；若独立验证中IM7始终远低于目标，可删去7阶降低复杂度 | 在20 dBm辨识，再加入23至25 dBm峰值样本；同功率验收要求IM3至少改善10 dB，IM5/IM7不得恶化超过1 dB |
| GMP | IM3/IM5/IM7为-41.22/-87.18/-129.50 dBc；默认 `nonlinearScale=0.135`，稳态压缩仍温和，但相位主导的长包络支路使动态失配显著 | 从正则化1/3/5阶、主深度3、交叉深度2的GMP开始并保留输入峰值投影；结合间隔验证决定是否增加7阶和更深长记忆 | 可在12至20 dBm建立局部逆，再用独立帧按相同输出功率验证；当前12 dBm基准实测IM3改善4.105 dB，不能把未实测的20 dBm改善写成既定结论 |
| Doherty | IM3/IM5/IM7为-39.09/-53.25/-62.30 dBc，属于轻度非线性；IM7仍明显高于Wiener，说明Peaking合成不能只按IM3判断 | 使用Carrier/Peaking分段DPD，例如平滑LUT负责主AM-AM/AM-PM，再由每区1/3/5/7阶GMP修正残差 | 对Peaking开启前、过渡区和开启后平衡取样，联合优化两区并惩罚边界不连续；验收需同时观察三个互调阶次，不能只优化IM3 |
...
| PA | 测试结论 | 针对性DPD结构与初始参数 | 训练和验收建议 |
|---|---|---|---|
| Rapp | 首个强失真采样点为24.76 dBm，此时IM3为-27.13 dBc；此前各点仍低于-30 dBc门限 | 在整个可逆功率范围共享一个静态LUT或深度1多项式；只把10/15/20/23/25 dBm作为覆盖与验收锚点，不为固定Rapp参数建立无意义的记忆系数库 | 对接近膝点的幅度箱加权并限制最大驱动；若目标输出已经进入近饱和区，应执行输出回退，而不是用更高阶或更大输入峰值强行求逆 |
| Wiener | 首个强失真采样点为24.87 dBm，此时IM3为-29.18 dBc；动态AM-AM仍约0.061 dB | 使用功率条件化Memory Polynomial，在10、15、20、23、25 dBm建立系数锚点并在锚点间插值；23 dBm以上加强峰值限制 | 20 dBm作为主训练点，23至25 dBm增加样本权重。若25 dBm的DPD输入峰值或EVM发散，应把额定部署点限制在可验证区而不是外推系数 |
| GMP | IM3从10至25 dBm单调恶化；首个强失真点为23.10 dBm，此时IM3仍为-35.13 dBc，但动态AM-PM已升至1.817度；25.10 dBm时IM3为-31.05 dBc、动态AM-PM为2.946度 | 10至25 dBm可先验证一个含长包络候选的GMP系数集；只有跨功率独立验证显示单一系数不足时，才使用按实测输出功率和包络RMS索引的系数库并约束相邻系数平滑 | 先在12或20 dBm建立稳定模型，再逐级验证23和25 dBm；只有跨功率独立EVM或ACLR确实退化时才引入多功率模型，不能由PA名称直接推定必须分库 |
| Doherty | 五个扫描点均未越过强失真门限；最高实测25.13 dBm时IM3为-37.73 dBc，且20/23/25 dBm的IM3为-39.09/-37.40/-37.73 dBc，呈轻微非单调 | 使用Carrier-only与Carrier+Peaking混合专家，保留10、15、20、23、25 dBm锚点，并在实测Peaking开启膝点附近按需加密；门函数应与实际开启点对齐 | 所有功率点联合训练但对过渡区和最高功率点加权，约束相邻功率系数和输出斜率连续；必须联合验收EVM、ACLR和IM3/IM5/IM7，不能由单个互调阶次推断宽带性能 |

统一验收规则是：在每个配置功率点重新闭环到相同实测dBm，双音侧要求IM3/IM5/IM7满足预先声明的改善或不退化门限，Wi-Fi侧分别为EVM和最差ACLR设置明确门限，且DPD输入峰值不超过硬件和定点接口允许范围。对于多功率折中，不得把容限内的ACLR轻微退化写成改善。系数只能在已经测量的功率范围内插值，不应向范围外外推。
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| Rapp | 0.000 | 0.000 | 0.000 | 0.000 | 0.052 | 0.000 | 0.000000 | 0.000 |
| Wiener | 0.315 | 0.285 | 0.148 | 0.629 | 0.007 | 0.001 | 0.061 | 0.014 |
| GMP | 2.050 | 0.202 | 0.080 | 0.567 | 11.084 | 11.740 | 0.028 | 0.898 |
| Doherty | 0.315 | 0.285 | 0.148 | 0.629 | 0.013 | 0.034 | 0.061 | 0.015 |

4 MHz标称点的互调结果为：

| PA模型 | 实际输出功率(dBm) | IM3(dBc) | IM5(dBc) | IM7(dBc) |
|---|---:|---:|---:|---:|
| Rapp | 19.977 | -56.288 | -65.921 | -83.244 |
| Wiener | 20.062 | -40.934 | -63.568 | -85.759 |
| GMP | 20.087 | -41.219 | -87.179 | -129.499 |
| Doherty | 20.071 | -39.091 | -53.246 | -62.302 |
...
|---|---:|---:|---:|---:|---:|---:|---:|
| Rapp | 10 | 10.000 | -116.201 | -125.726 | -142.644 | 0.000 | 0.000 |
| Rapp | 20 | 19.977 | -56.288 | -65.921 | -83.244 | 0.000000 | 0.000 |
| Rapp | 25 | 24.761 | -27.132 | -39.863 | -60.971 | 0.000011 | 0.000 |
| Wiener | 10 | 10.063 | -58.685 | -102.225 | -143.827 | 0.061 | 0.016 |
| Wiener | 20 | 20.062 | -40.934 | -63.569 | -85.760 | 0.061 | 0.014 |
| Wiener | 25 | 24.869 | -29.179 | -41.149 | -68.171 | 0.061 | 0.011 |
| GMP | 10 | 10.081 | -61.312 | -127.348 | -186.813 | 0.013 | 0.109 |
| GMP | 20 | 20.089 | -41.215 | -87.170 | -129.485 | 0.028 | 0.898 |
| GMP | 25 | 25.099 | -31.051 | -66.790 | -98.140 | 0.074 | 2.946 |
| Doherty | 10 | 10.063 | -62.999 | -110.941 | -157.016 | 0.061 | 0.016 |
| Doherty | 20 | 20.071 | -39.091 | -53.245 | -62.304 | 0.061 | 0.015 |
| Doherty | 25 | 25.131 | -37.735 | -42.534 | -56.638 | 0.062 | 0.013 |

目标与实测最大绝对误差约为0.239 dB，满足默认0.25 dB闭环容限。当前Doherty的IM3从20 dBm的-39.09 dBc恶化到23 dBm的-37.40 dBc，随后在25.13 dBm轻微改善到-37.73 dBc；同期IM5从-53.25 dBc恶化到-42.53 dBc，IM7则从-62.30 dBc变化到-56.64 dBc。支路复数合成仍具有功率和阶次选择性，不能只用一个互调阶次评价整体线性度，也不能强制曲线单调。
...
- GMP默认配置以 `nonlinearScale=0.135` 缩放三阶及以上的完整参考稳态系数，再把每一阶目标作为所有支路的系数和，并叠加零和的短记忆项与12抽头三阶长包络支路。因此连续高幅平台的稳态响应不会随记忆深度重复压缩；增强后的相位主导长支路使间隔依赖、侧带不对称和动态AM-PM明确可见，而动态AM-AM仍较小。只有显式配置 `nonlinearScale=1.0` 才选择 full-reference 稳态压力曲线。
- Doherty小信号时Peaking关闭，所以频响与Carrier所用默认Wiener一致。20 dBm时Peaking已经参与合成，但当前平滑默认配置的IM3/IM5/IM7为-39.09/-53.25/-62.30 dBc。默认Peaking没有额外时延，故间隔变化和动态迟滞仍较小；功率扫描中的轻微非单调来自两支路复数合成。
- 默认GMP在20.09 dBm处IM3为-41.21 dBc；23.10 dBm时IM3仍为-35.13 dBc，但动态AM-PM已升至1.817度，因而由动态门限进入强失真分类；到25.10 dBm时IM3为-31.05 dBc、动态AM-PM为2.946度。该结果只说明当前默认系数与长记忆支路的行为，不表示GMP结构天然优于或劣于Wiener、Doherty。用实测PA拟合系数或显式 full-reference 系数替换默认值后，应重新生成全部表格。
...
```mermaid
flowchart TD
    paResult["GMP PA特性：单调压缩、确定性长包络记忆、功率敏感"] --> label["10/12/14 dBm分别运行ILC生成PA输入标签"]
    ...
    basic --> metrics["seed 987独立帧同功率Wi-Fi EVM/ACLR和双音IM3/5/7"]
    memory --> metrics
    peak --> metrics
    ridge --> metrics
    multi --> metrics
...
**图6说明：**每个模型阶段都在DPD加PA的完整串联系统上重新闭环输入，PA输出不做事后缩放。seed 321只生成ILC标签和拟合系数，seed 987专门测量Wi-Fi EVM与ACLR。扩展结构、峰值加权、正则化和多功率训练分别针对不同问题，因此各自使用与设计目标一致的验收指标。
...
| Wi-Fi训练帧 | EHT、20 MHz、80 MS/s、MCS 7、4个数据符号、seed 321 |
| Wi-Fi独立验证帧 | 与训练帧相同PHY参数、seed 987；不参与ILC标签生成或DPD拟合 |
...
12 dBm是当前基准的标称训练点，15 dBm是同一系数结构的较高功率压力点。第6节的新功率扫描显示默认GMP在15.08 dBm时IM3为-51.29 dBc，20.09 dBm时为-41.21 dBc，23.10 dBm才因动态AM-PM达到1.817度进入强失真分类；因此12和15 dBm都处于单调、可稳定求逆的区域。二者的比较用于量化功率回退带来的误差裕量，不表示15 dBm已经深压缩。

### 12.2 全阶段结果

![DPD-GMP性能改进](./images/pa_analyse/dpd_gmp/dpd_gmp_performance.png)

**图7说明：**左上为seed 987独立验证帧的同输出功率Wi-Fi EVM，右上为同输出功率双音IM3，左下为seed 321训练标签的普通/峰值加权NMSE，右下为正则矩阵条件数。两条PA baseline没有标签和系数，所以后两类值为空。条件数使用对数坐标。验证帧不参与ILC标签生成或系数拟合，因而EVM与ACLR不会复用训练帧给出偏乐观结果。

| 阶段 | 实测输出(dBm) | EVM(dB) | ACLR(dB) | IM3(dBc) | 12 dBm标签NMSE(dB) |
|---|---:|---:|---:|---:|---:|
| PA baseline stress | 15.086 | -42.123 | 32.869 | -47.605 | — |
| PA baseline nominal | 12.084 | -46.548 | 32.990 | -53.638 | — |
| Basic DPD-GMP stress | 15.083 | -46.644 | 32.970 | -51.616 | -60.226 |
| Basic DPD-GMP nominal | 12.082 | -51.700 | 33.049 | -57.743 | -60.226 |
| Memory-expanded DPD-GMP | 12.082 | -52.428 | 33.059 | -58.802 | -63.971 |
| Peak-weighted DPD-GMP | 12.082 | -52.532 | 33.086 | -59.499 | -63.567 |
| Regularized DPD-GMP | 12.082 | -52.373 | 33.046 | -59.123 | -63.351 |
| Multi-power DPD-GMP | 12.082 | -51.988 | 33.044 | -58.775 | -62.416 |

这张表不能按单一纵轴从上到下选“最好模型”。后续阶段有意改变目标：Memory-expanded追求结构表达能力，Peak-weighted追求峰值误差，Regularized追求数值稳定，Multi-power追求多个工作点的最差性能。

### 12.3 改进一：建立基础局部逆

**PA依据：**默认GMP的稳态AM-AM在声明的归一化幅度范围内单调；功率扫描在23.10 dBm前未越过强失真门限，23.10 dBm也仍有-35.13 dBc IM3，只因动态AM-PM升至1.817度进入强失真分类。因此12 dBm局部映射可稳定求逆。
...
|---|---:|---:|---:|---|
| Wi-Fi EVM | -46.548 dB | -51.700 dB | 5.152 dB | 是，EVM更负 |
| 双音IM3 | -53.638 dBc | -57.743 dBc | 4.105 dB | 是，IM3更负 |
| ACLR | 32.990 dB | 33.049 dB | 0.058 dB | 是，ACLR更高 |

Wi-Fi训练的系数不仅在独立seed的Wi-Fi帧上改善EVM与ACLR，而且在没有双音重训练的情况下仍改善IM3，说明模型学到的是可迁移的PA局部逆，而不只是记住一组训练样点。

### 12.4 改进二：较高功率压力点执行输出回退

**PA依据：**功率扫描中15.08 dBm的IM3约-51.29 dBc，仍未进入深压缩，但相较12 dBm有更大的非线性误差。降低输出功率会同时增加局部斜率裕量和峰值余量，因此应能进一步降低EVM。
...
|---|---:|---:|---:|---|
| Wi-Fi EVM | -46.644 dB | -51.700 dB | 5.056 dB | 是 |
| ACLR | 32.970 dB | 33.049 dB | 0.079 dB | 是 |

在15 dBm处，基础DPD仍把独立验证帧上的同功率PA baseline EVM从-42.123 dB改善到-46.644 dB，改善4.521 dB，说明该点可由当前受限结构稳定补偿。回到12 dBm后的额外5.056 dB改善证明的是功率回退收益，而不是15 dBm不可逆。

### 12.5 改进三：扩展非线性和交叉记忆

**PA依据：**默认GMP的IM3间隔变化11.084 dB、最大上下侧不对称11.740 dB、动态AM-PM迟滞0.898度，长包络记忆已明确可测。扩展结构用于量化有限深度GMP能覆盖多少动态，但默认交叉深度3仍短于PA的12抽头支路。
...
|---|---:|---:|---:|---|
| 12 dBm标签NMSE | -60.226 dB | -63.971 dB | 3.745 dB | 是，表达能力提高 |
| Wi-Fi EVM | -51.700 dB | -52.428 dB | 0.728 dB | 小幅改善 |

本项预期是“更准确描述ILC标签”，不是保证射频指标同比例下降。标签NMSE改善3.745 dB，而独立帧EVM只改善0.728 dB；有限交叉深度仍未覆盖12抽头PA支路，所以标签收益不会等比例转化为射频EVM。
...
|---|---:|---:|---:|---|
| 峰值加权标签NMSE | -65.168 dB | -65.778 dB | 0.610 dB | 是 |
| 普通标签NMSE | -63.971 dB | -63.567 dB | -0.403 dB | 允许的权衡 |
...
| Wi-Fi EVM | -52.532 dB | -52.373 dB | -0.159 dB | 允许的小幅代价 |

本项设计目标是稳定性。训练误差或独立帧EVM的小幅牺牲不能否定条件数降低；应进一步用重复采集和定点量化验证系数方差。
...
|---|---:|---:|---:|---|
| 10/12/14 dBm最差标签NMSE | -55.362 dB | -57.121 dB | 1.759 dB | 是 |
| 10/12/14 dBm最差ACLR | 33.026863 dB | 33.019261 dB | -0.007601 dB | 是，退化未超过0.10 dB容限 |
| 10/12/14 dBm最差EVM | -48.594 dB | -49.115 dB | 0.521 dB | 跨功率最差点改善 |
| 12 dBm EVM | -52.373 dB | -51.988 dB | -0.385 dB | 单点性能折中 |

多功率模型改善了预先声明的最差标签NMSE；独立验证帧的最差ACLR下降0.007601 dB，满足“不退化超过0.10 dB”的预设容限。与此同时，跨功率最差EVM改善0.521 dB，但12 dBm单点EVM退化0.385 dB。这不是“全面更好”，而是当前1/2/1权重下的明确折中。这说明下一步有两种选择：
