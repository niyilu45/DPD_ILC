...
| GMP | IM3间隔变化1.730 dB、最大不对称0.520 dB；12抽头零稳态和长包络支路产生明确的频率相关记忆 | 从1/3/5阶、主记忆深度3、交叉深度2的GMP开始，并把长lagging-envelope支路或等效包络状态作为候选 | 各间隔保持相同实际输出功率，用未参与拟合的中间间隔验证；若短记忆残差仍超过测量重复性，再增加lagging/leading深度，且IM5/IM7不能恶化超过1 dB |
| Doherty | IM3间隔变化0.012 dB、最大不对称0.040 dB；默认支路没有明显额外时延，但Carrier/Peaking合成仍产生弱侧带差 | 使用每个工作区奇数阶1/3/5/7、深度3的浅记忆模型，并在Carrier/Peaking区域之间使用连续平滑门控 | 除间隔扫描外增加Peaking开启附近的功率点；若真实硬件支路存在时延，再按实测结果增加支路专用记忆，不能仅依据本默认模型省略 |

关键原则是先由“间隔变化”和“侧带不对称”证明交叉记忆项确有必要，再增加模型自由度。当前默认GMP的1.730 dB与0.520 dB已经显著高于无记忆数值基线，支持测试更深的包络记忆；但最终深度仍应由独立间隔和Wi-Fi帧的验证收益决定，避免把大量弱可观测列无条件加入回归矩阵。
...
**图2说明：**左上为IM3随双音间隔的变化，右上为上下侧IM3绝对不对称；下方两图分别为动态AM-AM和AM-PM迟滞。Rapp提供接近0的无记忆数值基线；默认GMP的12抽头长包络支路使间隔变化和侧带差明显增大，但零稳态和仍让幅相迟滞保持在较小范围，因此频域记忆与同幅度迟滞不能混为一个指标。
...
| GMP | 动态AM-AM为0.017 dB、AM-PM为0.052度；同幅度迟滞仍小，但间隔扫描已显示长包络记忆 | 保留lagging/leading支路并验证12抽头长包络候选；只有独立帧有收益时才采用完整深度或紧凑包络状态 | 训练集按幅度箱和包络方向均衡抽样，同时用未训练间隔检查频域残差；不能仅凭迟滞低于0.10 dB和1度就删除长记忆 |
| Doherty | 动态AM-AM为0.062 dB、AM-PM为0.012度；默认模型迟滞弱，但Peaking切换可能在真实硬件产生方向相关性 | 使用Carrier-only和Carrier+Peaking两个系数专家，以连续门函数平滑混合；初始每个专家深度3 | 在Peaking门限附近提高样本密度，并约束两套模型在边界处的函数值和一阶斜率连续，避免DPD自身制造迟滞环 |

对于当前默认GMP，20 dBm迟滞虽低于0.10 dB和1度门限，但1.730 dB的IM3间隔变化已经证明存在长包络动态；采用深GMP还是紧凑状态模型仍须由独立EVM、间隔扫描和标签NMSE共同决定。对于Doherty，重点仍不是无条件加深记忆，而是防止区域切换不连续。
...
| GMP | IM3/IM5/IM7为-49.51/-87.22/-129.54 dBc；默认 `nonlinearScale=0.135`，20 dBm仍是温和、单调的Rapp型压缩 | 从正则化1/3/5阶、主深度3、交叉深度2的GMP开始并保留输入峰值投影；结合间隔验证决定是否增加7阶和更深长记忆 | 可在12至20 dBm建立局部逆，再用独立帧按相同输出功率验证；当前12 dBm基准实测IM3改善4.835 dB，不能把未实测的20 dBm改善写成既定结论 |
| Doherty | IM3/IM5/IM7为-31.93/-46.03/-52.13 dBc，20 dBm已由旧默认配置的严重失真改善为中等非线性；IM7仍明显高于Wiener，说明Peaking合成不能只按IM3判断 | 使用Carrier/Peaking分段DPD，例如平滑LUT负责主AM-AM/AM-PM，再由每区1/3/5/7阶GMP修正残差 | 对Peaking开启前、过渡区和开启后平衡取样，联合优化两区并惩罚边界不连续；验收需同时观察三个互调阶次，不能只优化IM3 |
...
**图4说明：**左上为IM3，右上为IM5和IM7；下方为动态AM-AM和AM-PM迟滞。Rapp的互调随压缩增强，但AM-PM始终为0；Wiener在接近25 dBm时压缩迅速增强；GMP的IM3从10至25 dBm单调接近基波，长包络支路使动态AM-PM随功率由0.036度增至0.105度但仍保持平滑；Doherty在Peaking开启前与Wiener一致，开启后形成独立但平滑的功率轨迹，其IM3也随功率单调接近基波，不过IM5和IM7会因支路复数合成呈现不同的阶次趋势。Rapp在25 dBm出现的非零AM-AM“迟滞”来自固定宽度幅度分箱内的曲线陡峭度和有限样点分布，不是模型获得了历史状态。
...
| GMP | 五个扫描点均未越过-30 dBc强失真门限；25.10 dBm时IM3仍为-40.17 dBc、动态AM-PM为0.105度，且IM3功率趋势单调 | 10至25 dBm可先验证一个含长包络候选的GMP系数集；只有跨功率独立验证显示单一系数不足时，才使用按实测输出功率和包络RMS索引的系数库并约束相邻系数平滑 | 先在12或20 dBm建立稳定模型，再逐级验证23和25 dBm；只有跨功率独立EVM或ACLR确实退化时才引入多功率模型，不能由PA名称直接推定必须分库 |
| Doherty | 首个强失真采样点为24.76 dBm，此时IM3为-15.98 dBc；10至25 dBm的IM3趋势单调，但23至25 dBm间IM5改善、IM7明显恶化，体现Carrier与Peaking的阶次相关复数合成 | 使用Carrier-only与Carrier+Peaking混合专家，保留10、15、20、23、25 dBm锚点，并在实测Peaking开启膝点附近按需加密；门函数应与实际开启点对齐 | 所有功率点联合训练但对过渡区和最高功率点加权，约束相邻功率系数和输出斜率连续；必须联合验收EVM、ACLR和IM3/IM5/IM7，不能由单个互调阶次推断宽带性能 |
...
| GMP | 2.050 | 0.201 | 0.080 | 0.567 | 1.730 | 0.520 | 0.017 | 0.052 |
| Doherty | 0.315 | 0.285 | 0.148 | 0.629 | 0.012 | 0.040 | 0.062 | 0.012 |
...
| GMP | 20.089 | -49.514 | -87.223 | -129.544 |
| Doherty | 20.072 | -31.929 | -46.027 | -52.126 |
...
| GMP | 10 | 10.081 | -69.310 | -127.353 | -186.816 | 0.012 | 0.036 |
| GMP | 20 | 20.089 | -49.513 | -87.220 | -129.539 | 0.017 | 0.052 |
| GMP | 25 | 25.099 | -40.169 | -66.949 | -98.303 | 0.033 | 0.105 |
| Doherty | 10 | 10.063 | -51.463 | -87.443 | -121.364 | 0.061 | 0.016 |
| Doherty | 20 | 20.071 | -31.930 | -46.026 | -52.129 | 0.062 | 0.012 |
| Doherty | 25 | 24.757 | -15.977 | -39.656 | -32.128 | 0.060 | 0.011 |

目标与实测最大绝对误差约为0.247 dB，满足默认0.25 dB闭环容限。当前Doherty的IM3从20 dBm的-31.93 dBc单调恶化到25 dBm的-15.98 dBc，不再出现旧默认参数中的反常改善；但同期IM5从23 dBm的-30.92 dBc改善到25 dBm的-39.66 dBc，而IM7从-51.24 dBc恶化到-32.13 dBc，说明支路复数合成仍具有阶次选择性，不能只用一个互调阶次评价整体线性度。
...
- GMP默认配置以 `nonlinearScale=0.135` 缩放三阶及以上的完整参考稳态系数，再把每一阶目标作为所有支路的系数和，并叠加零和的短记忆项与12抽头三阶长包络支路。因此连续高幅平台的稳态响应不会随记忆深度重复压缩；长支路使间隔依赖和侧带不对称明确可见，同时动态迟滞仍保持平滑。只有显式配置 `nonlinearScale=1.0` 才选择 full-reference 压力曲线。
- Doherty小信号时Peaking关闭，所以频响与Carrier所用默认Wiener一致。20 dBm时Peaking已经参与合成，但当前平滑默认配置把IM3/IM5控制在与单支路Wiener相近的中等非线性范围；IM7仍相对较高。默认Peaking没有额外时延，故间隔变化和动态迟滞仍较小。
- 默认GMP在20.09 dBm处仍为温和非线性，IM3为-49.51 dBc；到25.10 dBm时IM3也仍为-40.17 dBc，五个默认扫描点均未进入本基准的-30 dBc强失真区。该结果只说明当前默认系数与长记忆支路的行为，不表示GMP结构天然优于或劣于Wiener、Doherty。用实测PA拟合系数或显式 full-reference 系数替换默认值后，应重新生成全部表格。
...
12 dBm是当前基准的标称训练点，15 dBm是同一系数结构的较高功率压力点。第6节的新功率扫描显示默认GMP到20.09 dBm时IM3仍为-49.51 dBc，25.10 dBm时也为-40.17 dBc；因此12和15 dBm都处于单调、可稳定求逆的温和失真区域。二者的比较用于量化功率回退带来的误差裕量，不表示15 dBm已经深压缩。
...
|---|---:|---:|---:|---:|---:|
| PA baseline stress | 15.085 | -48.303 | 33.027 | -58.379 | — |
| PA baseline nominal | 12.084 | -50.289 | 33.037 | -64.355 | — |
| Basic DPD-GMP stress | 15.083 | -53.447 | 33.062 | -63.721 | -67.632 |
| Basic DPD-GMP nominal | 12.082 | -56.024 | 33.075 | -69.190 | -67.632 |
| Memory-expanded DPD-GMP | 12.082 | -56.437 | 33.071 | -70.645 | -70.877 |
| Peak-weighted DPD-GMP | 12.082 | -56.448 | 33.080 | -71.787 | -70.195 |
| Regularized DPD-GMP | 12.082 | -56.318 | 33.063 | -71.377 | -69.882 |
| Multi-power DPD-GMP | 12.082 | -55.980 | 33.062 | -69.618 | -68.751 |

这张表不能按单一纵轴从上到下选“最好模型”。后续阶段有意改变目标：Memory-expanded追求结构表达能力，Peak-weighted追求峰值误差，Regularized追求数值稳定，Multi-power追求多个工作点的最差性能。

### 12.3 改进一：建立基础局部逆

**PA依据：**默认GMP的稳态AM-AM在声明的归一化幅度范围内单调，且默认扫描到25.10 dBm仍未越过-30 dBc强失真门限，因此12 dBm局部映射可稳定求逆。
...
|---|---:|---:|---:|---|
| Wi-Fi EVM | -50.289 dB | -56.024 dB | 5.735 dB | 是，EVM更负 |
| 双音IM3 | -64.355 dBc | -69.190 dBc | 4.835 dB | 是，IM3更负 |
| ACLR | 33.037 dB | 33.075 dB | 0.038 dB | 是，ACLR更高 |

Wi-Fi训练的系数不仅在独立seed的Wi-Fi帧上改善EVM与ACLR，而且在没有双音重训练的情况下仍改善IM3，说明模型学到的是可迁移的PA局部逆，而不只是记住一组训练样点。

### 12.4 改进二：较高功率压力点执行输出回退

**PA依据：**功率扫描中15.08 dBm的IM3约-59.36 dBc，仍未进入深压缩，但相较12 dBm有更大的非线性误差。降低输出功率会同时增加局部斜率裕量和峰值余量，因此应能进一步降低EVM。
...
|---|---:|---:|---:|---|
| Wi-Fi EVM | -53.447 dB | -56.024 dB | 2.577 dB | 是 |
| ACLR | 33.062 dB | 33.075 dB | 0.013 dB | 是 |

在15 dBm处，基础DPD仍把独立验证帧上的同功率PA baseline EVM从-48.303 dB改善到-53.447 dB，改善5.144 dB，说明该点可由当前受限结构稳定补偿。回到12 dBm后的额外改善证明的是功率回退收益，而不是15 dBm不可逆。

### 12.5 改进三：扩展非线性和交叉记忆

**PA依据：**默认GMP的IM3间隔变化1.730 dB、最大上下侧不对称0.520 dB、动态AM-PM迟滞0.052度，长包络记忆已明确可测。扩展结构用于量化有限深度GMP能覆盖多少动态，但默认交叉深度3仍短于PA的12抽头支路。
...
|---|---:|---:|---:|---|
| 12 dBm标签NMSE | -67.632 dB | -70.877 dB | 3.245 dB | 是，表达能力提高 |
| Wi-Fi EVM | -56.024 dB | -56.437 dB | 0.413 dB | 小幅改善 |

本项预期是“更准确描述ILC标签”，不是保证射频指标同比例下降。标签NMSE改善3.245 dB，而独立帧EVM只改善0.413 dB；有限交叉深度仍未覆盖12抽头PA支路，所以标签收益不会等比例转化为射频EVM。
...
|---|---:|---:|---:|---|
| 峰值加权标签NMSE | -72.442 dB | -73.290 dB | 0.848 dB | 是 |
| 普通标签NMSE | -70.877 dB | -70.195 dB | -0.682 dB | 允许的权衡 |
...
| Wi-Fi EVM | -56.448 dB | -56.318 dB | -0.131 dB | 允许的小幅代价 |
...
|---|---:|---:|---:|---|
| 10/12/14 dBm最差标签NMSE | -61.790 dB | -64.010 dB | 2.220 dB | 是 |
| 10/12/14 dBm最差ACLR | 33.062276 dB | 33.061390 dB | -0.000886 dB | 是，退化未超过0.10 dB容限 |
| 10/12/14 dBm最差EVM | -54.946 dB | -54.774 dB | -0.172 dB | 跨功率EVM折中 |
| 12 dBm EVM | -56.318 dB | -55.980 dB | -0.337 dB | 单点性能折中 |

多功率模型改善了预先声明的最差标签NMSE；独立验证帧的最差ACLR只下降0.000886 dB，满足“不退化超过0.10 dB”的预设容限。与此同时最差EVM和12 dBm单点EVM都略有退化。这不是“全面更好”，而是当前1/2/1权重下的明确折中。这说明下一步有两种选择：
