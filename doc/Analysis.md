...
代码中的 `Analysis.CalculateEvmAlignedMse` 实现上述完整算子，`ILCIteration.evmAlignedMse` 保存每轮的无量纲平方误差，`ILCIteration.evmDb` 保存它的 dB 值。该计算器不属于 `ILCConfig`；标准 SISO 主流程和 `tests/BenchMark.py` 把它作为ILC入口的独立参数传入，因此最佳轮次仍按照与最终EVM相同的目标选择。ILC结束后，PA输出再交给 `resultAnalysis.Analyze`，独立得到SNR、EVM和ACLR。
