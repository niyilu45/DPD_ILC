...
默认2.0是面向20 dBm高PAPR基线的精度/余量折中：当前默认GMP在该点无rail并保持约 -32.15 dB本征EVM。接近25 dBm时若2.0出现rail，应让plant与Analysis同时使用4.0；当前边界复测实测25.088 dBm、EVM约 -22.09 dB且I/Q rail计数为0。TwoToneAnalysis分析同一固定点PA输出时也必须使用同一输出标尺；扩大标尺只恢复观测余量，不会改善PA本征EVM。
...
| 默认GMP浮点20 dBm约 -32.15 dB，16位FS1却约 -23.72 dB | PA输出被旧标尺1.0削顶 | 保持输入DAC标尺1.0；把PA/Channel输出和Analysis的 `outputFullScaleAmplitude` 同时设为2.0 |
...
上述FS1约 -23.72 dB现象不是“固定点必然明显差于浮点”。默认EHT MCS5、`seed=91`、无噪声20 dBm工作点的原始输出分量峰值约1.58；若仍按 `fullScaleAmplitude=1` 编码，超范围峰值会不可逆夹到码轨。内置PA/Channel默认输出标尺2.0后，16位结果回到约 -32.15 dB并接近浮点本征；增加位宽但保留标尺1.0不会修复范围问题。
