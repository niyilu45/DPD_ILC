...
本轮优化面向长波形、MIMO和重复ILC评估中最耗时的数组运算。原则是保持物理模型、公开接口、数组形状、定点边界和指标定义不变，只减少Python逐样点循环、重复FFT、重复OFDM解调、重复延迟构造以及理想模块的无效运算。
...
| 默认理想forward `Channel.Process` | 58.3 ms | 17.3 ms | 理想I/Q、零相位和零驱动旁路，加上GMP复用 |
