...
浮点归一化后的待编码波形总 RMS 为 1。这个数值归一化不代表真实端口上已经产生 1 W 或某个固定 dBm；它只提供统一的 PA 驱动基准。`width=0` 直接公开该单位RMS波形。若 `width>0`，生成器会先对整包、全部字段和全部天线统一乘一个峰值余量系数 $h\leq1$，使 I、Q 的正负峰值都落入有符号定点范围，再做唯一一次编码。解码后的固定点波形RMS约为 $h$，但波形形状、PAPR、空间映射和字段相对功率保持不变；`normalizationScale` 与参考数据星座同步包含这个系数，所以分析端不会把数字余量误判为EVM。
...
例如目标输出 20 dBm、额定极限 25 dBm 时，第一次驱动比例约为 0.5623；目标达到 25 dBm 时，第一次驱动比例为 1。由于PA具有小信号增益、压缩、记忆效应和定点量化，这个名义比例通常不能一次得到准确目标功率。`PowerCalibration` 会把重新生成的输入反复送入PA，测量有效突发输出功率，并在内部修正驱动预设，直到误差进入容限。PA输出不做常数重标定，所以EVM、SNR和ACLR对应真实工作点。
...
浮点模式直接输出 $x_{\mathrm{norm}}[n]$。定点模式先根据整包所有 I/Q 分量
计算一个统一的峰值安全系数。令正端物理上限为

```math
p_W=1-2^{-(W-1)},
```

负端幅度上限为 1，正、负分量峰值分别为 $a_+$、$a_-$，则
...
同一个 $h$ 乘到整包、所有天线和参考数据星座。随后编码为整数码：

```math
q[n]=
Q_W\left(
2^{W-1}h x_{\mathrm{norm}}[n]
\right).
```

其中 $Q_W$ 对 I、Q 分量分别执行最近整数舍入和有符号饱和。生成器把 $h$
向零方向退一个浮点最小步长，避免乘法舍入恰好越过端点，因此正常生成路径
不会依赖饱和器削平OFDM峰值。当 $W=0$ 时不编码；当 $W>0$ 时公开数组仍为
`complex128`，但实部和虚部是整数码。14位码范围为 `-8192…8191`，16位码
范围为 `-32768…32767`。一位I/Q只有不对称的两级码，不能表示双极性OFDM，
因此 `WaveGenWifi(width=1)` 会被拒绝。
...
浮点模式可直接用该比例作为第一次驱动试探。定点模式不反复缩放已经峰值安全的
公开码，因为那会让低功率点损失有效位数。校准器为本次闭环建立一次合法数字
预设，各轮候选差值放在解码后的隐藏模拟 drive 中：
...
$G_d$ 由闭环在 dB 域更新，成功后提交给PA/Channel公开处理路径。这样生成端
的 $h$ 只提供数字峰值余量，不会限制可达的模拟输出功率，也不会把1、10、16、
20 dBm变成不同削顶程度的数字波形。若调用方在生成之后另行叠加、滤波或放大
公开码，仍应重新检查端点；生成器只能保证自身输出不削顶。

每次PA处理后，校准器从有效突发测得每路输出RMS，并与目标RMS比较。目标RMS电压为：
...
令第 $k$ 次实际测得功率为 $p_{\mathrm{meas}}^{(k)}$，闭环误差为

```math
e^{(k)}
=p_{\mathrm{out,dBm}}-p_{\mathrm{meas}}^{(k)}.
```

若误差尚未进入容限，校准器在dB域更新隐藏驱动预设，并重新生成输入、重新运行PA；形成上下括区后使用二分更新。对外的模拟功率单位是PA实际输出dBm，用户不需要感知更新后的内部预设。默认 $R=50$ Ω 时，20 dBm 对应 2.236068 V RMS，25 dBm 对应约 3.976354 V RMS。注意功率测量针对**有效突发**，会排除前后补零和较长静默；MIMO按每条PA链独立闭环，并等待所有链同时满足容限。
...
| `width` | 配置发送I/Q接口位宽；0为浮点，正数返回有符号整数码 | 默认16位；编码只发生在完整波形输出边界 |
...
| 接口编码 | `FixedPoint.EncodeComplex` | 与浮点模式同形状、同为 `complex128`，但I/Q分量是整数码 |
...
assert fixedWaveform.samples.real.max() <= 32767
assert fixedWaveform.samples.real.min() >= -32768
```

比特、QAM、空间映射、IFFT和RMS归一化仍使用浮点，只在 `WifiWaveform.samples` 输出边界编码为整数码；理想 `referenceDataSymbols` 不编码。舍入、饱和、码值缩放和OFDM高峰的详细推导见 [FixedPoint.md](./FixedPoint.md)。
