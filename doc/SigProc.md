...
对固定分数相位 $\mu$，也可以把归一化离散权重和相对理想时延的残差频响写成：

```math
w_{L,\mu}[k]
=
\frac{
\mathrm{sinc}(\mu-k)\,\mathrm{sinc}((\mu-k)/L)
}{
\sum_{r=-L+1}^{L}
\mathrm{sinc}(\mu-r)\,\mathrm{sinc}((\mu-r)/L)
},
\qquad k=-L+1,\ldots,L,
```
...
R_{L,\mu}(e^{j\omega})
=e^{-j\omega\mu}
\sum_{k=-L+1}^{L}w_{L,\mu}[k]e^{j\omega k}.
```

理想情况下，整个占用带内应满足 $|R|=1$ 且 $\angle R=0$。因此解析幅相扫描适合预筛候选 $L$，但最终选阶仍必须使用实际有限Wi-Fi帧做端到端EVM验收，因为符号边界与带外分量也会进入重采样误差。有限支持 $L$ 控制计算量和精度；过采样Wi-Fi信号的有效带宽低于Nyquist边缘，sinc类插值通常明显优于简单线性插值。

### 7.1 先区分半支持长度、抽头数和FIR阶数

公开参数 `interpolationHalfLength=L` 不是“L阶滤波器”。非整数采样位置实际使用的offset为

```math
k=-L+1,-L+2,\ldots,L,
```

因此每个分数相位最多使用 $2L$ 个候选抽头，对应每相位等效FIR阶数 $2L-1$。默认 `L=12` 表示24抽头、23阶等效FIR；当全部采样位置恰好为整数时，代码走精确索引旁路，不执行这24抽头。对 $C$ 条链、每链 $N$ 点，非整数重采样的主要计算量近似为 $O(2LCN)$，所以阶数评估必须同时记录运行时间和峰值内存。

还必须区分两个参考面：

- Channel注入端决定“物理时延怎样被生成”；公共 `channelDly` 由 `channelFractionalDelayFilterOrder` 控制，默认23阶归一化Lanczos、阶数1才是旧线性模式；FB专用 `fbFractionalDelaySamples` 仍使用低阶线性插值。两者都可能产生各自的有限阶频响。
- SigProc接收端的 $L$ 只决定“怎样把已经收到的波形重采样回参考网格”。增大 $L$ 不能逆转注入端已经造成的幅度下垂，也不会提高三点抛物线相关峰的时延估计精度。
...
```math
r_{\mathrm{sys}}=10^{E_{\mathrm{sys}}/10},
\qquad
r_{\mathrm{int}}=10^{E_{\mathrm{int}}/10},
```

其中 $E$ 是越负越好的EVM dB。把两种残差按独立误差预算相加时，插值器使总EVM恶化

```math
\Delta E
=10\log_{10}
\left(1+10^{-M/10}\right),
\qquad
M=E_{\mathrm{sys}}-E_{\mathrm{int}}.
```

反过来，若最多允许总EVM恶化 $\Delta E_{\max}$，所需余量为：

```math
M
=-10\log_{10}\!\left(10^{\Delta E_{\max}/10}-1\right).
```

| 插值残差低于系统目标的余量 $M$ | 总EVM约恶化 |
|---:|---:|
| 10 dB | 0.414 dB |
| 15 dB | 0.135 dB |
| 16.3 dB | 0.100 dB |
...
可把10 dB作为普通筛选门限，此时插值误差可能让最终EVM恶化约0.41 dB；要求更严格时，应按上式由允许退化反推。若要求恶化不超过0.1 dB，工程上取17 dB余量。例如要严格评价约 -50 dB的PA/DPD，已知真实时延条件下的接收插值残差应不高于约 -67 dB。这个绝对预算比“L比默认值大”更有意义，因为所需支持长度强烈依赖 $F_s/B_{\mathrm{occ}}$、最坏分数相位、SFO累计漂移和实际信号功率谱。

### 7.3 接收端阶数充分性的双路径试验

判断 $L$ 是否足够时，必须把“插值核精度”和“时延估计精度”分开：

```mermaid
flowchart LR
    truth["独立高精度真值时延<br/>零扩展FFT或超长sinc"] --> known["路径A：直接使用真实时延补偿"]
    truth --> estimated["路径B：相关峰估计后补偿"]
    known --> kernel["得到插值核EVM下限"]
    estimated --> total["得到估计器+插值器总EVM"]
    kernel --> compare["扫描L与2L、带宽和mu"]
    total --> compare
    compare --> budget{"绝对预算和收敛均通过？"}
    budget -->|是| accept["选择最小通过的L"]
    budget -->|否| diagnose["区分核阶数、估计偏差或边界问题"]
```

推荐验收步骤如下：

1. 在浮点、无PA、无噪声、无CFO/SFO/IQ误差条件下，用零扩展FFT相位斜坡或远高于被测长度的windowed-sinc生成真实时延。不能用被测的同一个 $L$ 同时生成和补偿，否则会形成“同算法自证”的inverse crime。
2. 给输入和capture保留足够前后guard，只在完整有效区或完整OFDM数据符号上算EVM。对长度为 $N$ 的记录，可先取 $N_{\mathrm{guard}}\geq |D_{\mathrm{int}}|+L_{\mathrm{inject}}+L_{\mathrm{receive}}+\lceil N|\epsilon_{\mathrm{ppm}}|/10^6\rceil+N_{\mathrm{margin}}$，再用边界样点残差确认余量。
3. 对最小支持的 $F_s/B_{\mathrm{occ}}$、最长帧和所有代表性Wi-Fi格式，让 $\mu$ 从0.05到0.95按0.05步进扫描；必须包含0.5，但不能假设所有高阶核的最坏点一定恰好是0.5。
4. 路径A直接使用已知真实时延重采样，移除一个公共最小二乘复增益后得到 $E_{\mathrm{known}}(L)$。它回答“插值核够不够”。
5. 路径B使用 `EstimateTimingOffsets` 的估计值和相同 $L$，得到 $E_{\mathrm{auto}}(L)$、时延偏差及SFO偏差。它回答“完整接收同步够不够”。
6. 将 $L$ 加倍到 $2L$。比较路径B时先冻结同一组已估计的时延/CFO/SFO，只替换插值核，不能让每个 $L$ 重新估计后发生偶然抵消。只有最坏 $E_{\mathrm{known}}$ 已满足绝对预算，且加倍后变化小于项目门限（推荐0.1 dB），才能把较小的 $L$ 判为足够。最后再检查完整路径B也满足系统预算。
7. 浮点验收通过后，再覆盖实际定点位宽、量化饱和和最长帧；同时记录每样点耗时与峰值临时内存，避免用无法部署的超长核换取没有系统价值的EVM余量。

### 7.4 怎样区分阶数不足、估计不准和上游失真

| 观察结果 | 结论 | 应采取的动作 |
|---|---|---|
| 路径A随 $L$ 增大持续改善，路径B也相近改善 | 接收插值支持不足 | 增大 `interpolationHalfLength`，直到绝对预算与收敛同时通过 |
| 路径A已经通过，但路径B平台化或变差 | 时延估计偏差成为瓶颈 | 提高相关峰估计精度或做数据辅助的分数时延一维优化，不要继续盲目加抽头 |
| 路径A和B都不随 $L$ 改善，残差集中在带边 | 注入端或真实链路已有频率选择性 | 修正Channel生成端，或使用独立频响均衡；接收重采样不是均衡器 |
| 加guard后EVM突然恢复 | capture边界被截断 | 扩展capture、只评价完整符号或保留跨块状态 |
| $L$ 与 $2L$ 结果接近但都未达到绝对预算 | 算法族或采样率不足 | 换更合适的核、提高采样率，不能仅凭“已收敛”判定通过 |

### 7.5 本工程的离线参考阶数基线

下表来自一次离线参考量化，不是仓库的固定回归fixture：使用EHT 20 MHz、MCS 9、20个数据symbol、浮点、无PA/噪声，采用与被测Lanczos不同的1025抽头Kaiser-windowed-sinc做线性卷积真值注入，并扫描 $\mu=0.10,0.25,0.50,0.75,0.90$。注入核加倍到2049抽头后结果变化不超过0.13 dB。数值是直接使用已知真实时延补偿时的扫描最差EVM，因此主要反映接收插值核；它们只能作为候选起点。正式验收必须固定并记录波形seed、窗参数、guard、整数时延、EVM评价区间、增益处理、运行环境和完整命令。

| 接收 $L$ | 抽头数 | 等效FIR阶数 | $F_s/B_{\mathrm{occ}}=2$ | $F_s/B_{\mathrm{occ}}=4$ |
|---:|---:|---:|---:|---:|
| 2 | 4 | 3 | -30.9 dB | -39.9 dB |
| 3 | 6 | 5 | -39.8 dB | -46.3 dB |
| 4 | 8 | 7 | -47.6 dB | -51.7 dB |
| 6 | 12 | 11 | -52.9 dB | -57.6 dB |
| 8 | 16 | 15 | -56.8 dB | -62.7 dB |
| 12 | 24 | 23 | -61.8 dB | -69.2 dB |
| 16 | 32 | 31 | -65.1 dB | -73.1 dB |
| 24 | 48 | 47 | -69.3 dB | -77.1 dB |
| 32 | 64 | 63 | -71.9 dB | -79.9 dB |
| 48 | 96 | 95 | -74.5 dB | -83.5 dB |

再让当前三点抛物线相关峰自动估计时延，并固定默认 $L=12$。下表的估计偏差是5个随机帧的有符号均值，EVM是这5帧中最差、即数值最高且最不负的一项：

| $\mu$ | 2倍采样5帧平均偏差 | 2倍采样5帧最差EVM | 4倍采样5帧平均偏差 | 4倍采样5帧最差EVM |
|---:|---:|---:|---:|---:|
| 0.10 | -0.0111 sample | -40.1 dB | -0.00305 sample | -57.4 dB |
| 0.25 | -0.0223 sample | -34.1 dB | -0.00601 sample | -51.3 dB |
| 0.50 | 约0 | -61.1 dB | 约0 | -69.0 dB |
| 0.75 | +0.0224 sample | -34.1 dB | +0.00603 sample | -51.5 dB |
| 0.90 | +0.0111 sample | -40.0 dB | +0.00307 sample | -57.3 dB |

因此，在约 -50 dB系统目标下，普通10 dB筛选可从2倍采样的 `L=12`、4倍采样的 `L=8` 开始；严格17 dB余量则应从2倍采样的 `L=24`、4倍采样的默认 `L=12` 开始。自动模式在 $\mu=0.25/0.75$ 处即使把 $L$ 提高到48，仍停在2倍采样约 -34.2 dB、4倍采样约 -51.5 dB，瓶颈是时延估计而不是抽头数。若2倍采样还要求低于约 -70 dB的插值底噪，与其继续暴力增加Lanczos长度，更应优先提高采样率或采用经带宽优化的Farrow/equiripple结构。

计算成本近似随 $L$ 线性增长。在同一台测试主机的4倍采样基准中，`L=12` 约为1.25 us/sample；`L=24/32/48` 分别约为默认耗时的1.95/2.76/4.12倍。绝对耗时随CPU和NumPy版本变化，正式选型应在目标硬件上复测。

### 7.6 时延估计精度也必须进入预算

令补偿后的剩余时延偏差为 $\Delta\tau$，信号的功率加权归一化角频率RMS为

```math
\sigma_\omega
=\sqrt{
\frac{\int S_x(\omega)\omega^2\,d\omega}
...
```math
\mathrm{EVM}_{\tau}
\approx |\Delta\tau|\sigma_\omega,
\qquad
|\Delta\tau|
\lesssim
\frac{10^{E_{\tau,\mathrm{budget}}/20}}{\sigma_\omega}.
```

若功率在 $[-\Omega_B,\Omega_B]$ 内近似均匀，且 $K=F_s/B_{\mathrm{occ}}$，则 $\sigma_\omega\approx\Omega_B/\sqrt3\approx\pi/(\sqrt3K)$，因此

```math
\mathrm{EVM}_{\tau}
\approx
\frac{\pi|\Delta\tau|}{\sqrt3K}.
```

例如4倍采样时，要让时延估计本身低于 -65 dB，偏差应约小于0.00124 sample；若预算为 -60 dB，2倍和4倍采样的偏差门限分别约为0.00110和0.00221 sample。当前基准在 $\mu=0.25/0.75$ 处约0.006 sample的偏差足以解释为什么增加 $L$ 后自动路径不再改善。

工程上可以通过零填充相关频谱提高峰值采样密度、在粗峰附近直接最小化数据辅助MSE、利用训练子载波相位斜率，或联合优化分数时延与公共复增益来降低偏差。真实接收数据没有可编程真值时，至少应对同一capture比较 $L$ 与 $2L$、比较不同时间窗和高低子带的时延估计，并观察每子载波残差是否在带边系统性上升。若这些结果不稳定，就不能宣称“接收端阶数已经足够”。
...
sigProc = SigProc(
    referenceSignal,
    sampleRateHz,
    parameters={"interpolationHalfLength": 12},
)
```

这里的12必须解释为半支持长度，即24抽头、23阶等效FIR。选阶报告至少应同时给出目标EVM、允许退化、由此得到的余量、最小OSR、最坏 $\mu$、已知时延EVM、自动EVM、时延偏差和 $L\rightarrow2L$ 的变化量。

以下做法不能证明阶数足够：

- 用同一个Lanczos核生成时延又补偿时延；
- 只测试容易出现对称抵消的 $\mu=0.5$；
- 把补零/截尾边界一起算入核误差，却不保留guard；
- 每次改变 $L$ 都重新估计时延，再把估计误差与插值误差的偶然抵消当成改善；
- 路径A已经通过后仍靠增加 $L$ 修复相关峰估计偏差，或修复Channel生成端在低阶配置、硬件通道和有限记录边界中已经形成的幅频失真；
...
`firOrder` 在这段独立代码中的含义始终是“抽头数减1”。一阶特例使用线性插值；三阶及以上的奇数阶使用Lanczos，因此比较一阶和高阶时同时改变了算法族。它与Channel的 `channelFractionalDelayFilterOrder` 使用相同的阶数、抽头数和内部Lanczos核约定；但本独立补偿函数会在有限记录边界按当前有效抽头重新归一化，Channel为了保持固定LTI系数则使用零扩展且不做边界重归一化。因此两者在拥有完整抽头支持的内部样点一致，边界样点不保证逐点相同。常用调用如下：
...
| `interpolationHalfLength` | `12` | Lanczos-sinc插值半支持长度 $L$，最小2；非整数位置最多使用 $2L$ 个抽头，默认即24抽头/23阶等效FIR。它只控制重采样核，不改变三点相关峰估计精度；选择与验收方法见§7 |
