...
为什么基函数是 $x|x|^{p-1}$？令 $x=re^{j\theta}$，则
...
当 $|x|\rightarrow0$ 时，$p>1$ 的项比一阶项衰减得更快：
...
**图 6 说明**：当 $\beta=0$ 时没有镜像；$|\beta|$ 越大，镜像越强。`IQImbalancePA` 把任意基础 PA 包装成这一形式，可测试普通复多项式 DPD 对非解析共轭失真的处理边界。
...
所以 $E[|w|^2]=P_n$。`AddAwgn` 实现的正是这一缩放。这里的 AWGN 表示反馈接收机热噪声/量化噪声的简化等效，不包含相位噪声、频偏、非线性 ADC 或相关噪声。
