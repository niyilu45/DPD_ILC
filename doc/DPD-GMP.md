...
| 特性随输出功率变化 | 用 `FitSegments` 联合多个功率标签 | 最差功率标签NMSE改善，独立帧EVM验收且ACLR不超过护栏 |
...
| 改进 | 改进前 | 改进后 | 目标变化 |
|---|---:|---:|---:|
| 基础 DPD-GMP：独立帧 Wi-Fi EVM | -46.548 dB | -51.700 dB | 改善 5.152 dB |
| 基础 DPD-GMP：双音 IM3 | -53.638 dBc | -57.743 dBc | 改善 4.105 dB |
| 15 dBm 压力点回到 12 dBm：基础 DPD EVM | -46.644 dB | -51.700 dB | 改善 5.056 dB |
| 扩展阶数与记忆：标签 NMSE | -60.226 dB | -63.971 dB | 改善 3.745 dB |
| 峰值加权：峰值标签 NMSE | -65.168 dB | -65.778 dB | 改善 0.610 dB |
| 增强岭正则：条件数 | 约 `5.44e7` | 约 `5.48e5` | 改善约 19.96 dB |
| 多功率联合训练：最差标签 NMSE | -55.362 dB | -57.121 dB | 改善 1.759 dB |
| 多功率联合训练：独立帧最差 ACLR | 33.026863 dB | 33.019261 dB | 退化 0.007601 dB，0.10 dB护栏内通过 |

扩展结构在当前长记忆默认模型上把独立验证帧的12 dBm Wi-Fi EVM从-51.700 dB改善到-52.428 dB，约改善0.728 dB，说明标签拟合收益并不等于同量级的射频收益。多功率ACLR也不是强制改善项：源Wi-Fi波形的ACLR本底约为33 dB，因此基准只要求多功率最差ACLR相对单功率正则模型的下降不超过0.10 dB。本次从33.026863 dB变为33.019261 dB，退化0.007601 dB并通过护栏，不能据此宣称ACLR得到提升。多功率最差EVM从-48.594 dB改善到-49.115 dB，但12 dBm单点EVM从-52.373 dB退化到-51.988 dB，体现联合训练的跨功率折中。精确机器可读数据位于 `doc/images/pa_analyse/dpd_gmp` 中的CSV和JSON。参考数值只描述当前默认行为模型、`seed=321`训练帧和`validationSeed=987`独立验证帧，真实PA必须重新采集和验收。
...
本节的“分段 GMP”指可部署的 `PiecewiseDpdGmp` 算法，不表示被测PA必须是
`PaModel(modelName="piecewise_gmp")`。功率差异验证可在普通GMP PA上分别训练
`DpdGmp` 与 `PiecewiseDpdGmp`；两种算法共享同一个plant，比较的是DPD结构。
内置 `piecewise_gmp` PA则是另一种行为plant，其标准主流程EVM约为-24.65 dB，
并保留较弱的长包络系数 `0.008-j0.0036`。不能把普通GMP PA增强后的长包络
默认值同步灌入该plant，否则会同时改变被测对象并误伤原有分段PA基准。
