...
| 改进 | 改进前 | 改进后 | 目标变化 |
|---|---:|---:|---:|
| 基础 DPD-GMP：独立帧 Wi-Fi EVM | -50.289 dB | -56.024 dB | 改善 5.735 dB |
| 基础 DPD-GMP：双音 IM3 | -64.355 dBc | -69.190 dBc | 改善 4.835 dB |
| 15 dBm 压力点回到 12 dBm：基础 DPD EVM | -53.447 dB | -56.024 dB | 改善 2.577 dB |
| 扩展阶数与记忆：标签 NMSE | -67.632 dB | -70.877 dB | 改善 3.245 dB |
| 峰值加权：峰值标签 NMSE | -72.442 dB | -73.290 dB | 改善 0.848 dB |
| 增强岭正则：条件数 | 约 `5.44e7` | 约 `5.48e5` | 改善约 19.96 dB |
| 多功率联合训练：最差标签 NMSE | -61.790 dB | -64.010 dB | 改善 2.220 dB |
| 多功率联合训练：独立帧最差 ACLR | 33.062276 dB | 33.061390 dB | 退化 0.000886 dB，0.10 dB护栏内通过 |

扩展结构在当前长记忆默认模型上把独立验证帧的12 dBm Wi-Fi EVM从-56.024 dB改善到-56.437 dB，约改善0.413 dB，说明标签拟合收益并不等于同量级的射频收益。多功率ACLR也不是强制改善项：源Wi-Fi波形的ACLR本底约为33 dB，因此基准只要求多功率最差ACLR相对单功率正则模型的下降不超过0.10 dB。本次从33.062276 dB变为33.061390 dB，退化0.000886 dB并通过护栏，不能据此宣称ACLR得到提升。精确机器可读数据位于 `doc/images/pa_analyse/dpd_gmp` 中的CSV和JSON。参考数值只描述当前默认行为模型、`seed=321`训练帧和`validationSeed=987`独立验证帧，真实PA必须重新采集和验收。
