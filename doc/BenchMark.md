...
- 普通GMP默认 `nonlinearScale=0.135`，三阶及以上只采用 full-reference 稳态系数的13.5%，并增加零稳态和的12抽头三阶长包络支路；20 dBm下IM3间隔变化1.730 dB、最大侧带不对称0.520 dB、动态AM-AM/AM-PM分别为0.017 dB和0.052度；
- GMP在20.09 dBm的IM3/IM5/IM7为-49.51/-87.22/-129.54 dBc，25.10 dBm时IM3仍为-40.17 dBc；五个默认扫描点都没有越过-30 dBc强失真门限。`nonlinearScale=1.0` 只是显式 full-reference 压力场景；
...
| 改进 | 前值 | 后值 | 默认改善 | 预期 |
|---|---:|---:|---:|---|
| 基础DPD独立帧Wi-Fi EVM | -50.289 dB | -56.024 dB | 5.735 dB | EVM降低 |
| 基础DPD双音IM3 | -64.355 dBc | -69.190 dBc | 4.835 dB | IM3降低 |
| 15至12 dBm功率回退 EVM | -53.447 dB | -56.024 dB | 2.577 dB | EVM降低 |
| 扩展结构标签NMSE | -67.632 dB | -70.877 dB | 3.245 dB | NMSE降低 |
| 峰值加权标签NMSE | -72.442 dB | -73.290 dB | 0.848 dB | 峰值目标降低 |
| 增强正则条件数 | `5.435e7` | `5.481e5` | 19.964 dB | 条件数降低 |
| 多功率最差标签NMSE | -61.790 dB | -64.010 dB | 2.220 dB | 最差值降低 |
| 多功率独立帧最差ACLR | 33.062276 dB | 33.061390 dB | 退化0.000886 dB | 退化不超过0.10 dB |

扩展结构、峰值加权和正则化不要求即时EVM都下降，因为三者分别优化标签表达、峰值误差和数值稳定。当前扩展结构额外改善0.413 dB独立帧EVM；长记忆支路超过扩展模型的交叉深度，所以增加有限深度后的射频收益仍然有限。多功率训练把最差标签NMSE改善2.220 dB，但独立帧最差ACLR下降0.000886 dB，且12 dBm EVM相对单功率正则模型退化0.337 dB；因此它是可量化的折中，不应描述为全面改善。源Wi-Fi波形的ACLR本底约为33 dB，ACLR验收采用“相对单功率正则模型退化不超过0.10 dB”的护栏；本次结果通过只表示没有明显频谱退化，不表示ACLR得到提升。
