...
| `saturationAmplitude` | 1.55 | Rapp压缩幅度尺度 |
...
|---|---:|---:|---:|---:|---:|---:|
| PA baseline | -40.93 | -63.56 | -85.75 | 0.00 | 0.00 | 0.00 |
| Scalar P ILC | -41.85 | -64.61 | -86.33 | 0.92 | 1.04 | 0.58 |
| Complex-gain ILC | -42.34 | -65.18 | -86.64 | 1.41 | 1.62 | 0.89 |
| FIR ILC | -42.38 | -65.16 | -86.66 | 1.45 | 1.59 | 0.91 |
| Frequency-domain ILC | -42.35 | -65.18 | -86.64 | 1.42 | 1.62 | 0.89 |
| Directional Gauss-Newton ILC | -41.00 | -63.64 | -85.79 | 0.07 | 0.08 | 0.04 |
| Parameter-domain MP ILC | -42.60 | -64.73 | -79.84 | 1.67 | 1.17 | -5.91 |
| Augmented IQ ILC | -40.95 | -64.06 | -86.45 | 0.01 | 0.50 | 0.70 |

快速结果证明了必须分阶次比较：两轮内多数方法同时改善IM3、IM5和IM7，但Parameter-domain MP ILC虽然改善IM3/IM5，却让IM7退化5.91 dB。正式评估应使用默认10轮，并至少增加输出功率扫描或不同PA模型复测。
...
- Rapp频响、群时延、侧带不对称和动态迟滞接近0，20 dBm标称IM3/IM5/IM7为-56.29/-65.92/-83.24 dBc，但互调仍随输出功率进入压缩而恶化；
- Wiener与Doherty在Peaking关闭的小信号区频响接近；20 dBm标称互调分别为-40.93/-63.57/-85.76 dBc和-39.09/-53.25/-62.30 dBc；
- 普通GMP默认 `nonlinearScale=0.135`，三阶及以上只采用 full-reference 稳态系数的13.5%，并增加零稳态和的12抽头三阶长包络支路；20 dBm下IM3间隔变化11.084 dB、最大侧带不对称11.740 dB、动态AM-AM/AM-PM分别为0.028 dB和0.898度，表现为相位主导的动态记忆；
- GMP在20.09 dBm的IM3/IM5/IM7为-41.22/-87.18/-129.50 dBc；首个强失真点为23.10 dBm，此时IM3仍为-35.13 dBc，但动态AM-PM已达1.817度；25.10 dBm时IM3为-31.05 dBc。`nonlinearScale=1.0` 只是显式 full-reference 稳态压力场景；
...
| 改进 | 前值 | 后值 | 默认改善 | 预期 |
|---|---:|---:|---:|---|
| 基础DPD独立帧Wi-Fi EVM | -46.548 dB | -51.700 dB | 5.152 dB | EVM降低 |
| 基础DPD双音IM3 | -53.638 dBc | -57.743 dBc | 4.105 dB | IM3降低 |
| 15至12 dBm功率回退 EVM | -46.644 dB | -51.700 dB | 5.056 dB | EVM降低 |
| 扩展结构标签NMSE | -60.226 dB | -63.971 dB | 3.745 dB | NMSE降低 |
| 峰值加权标签NMSE | -65.168 dB | -65.778 dB | 0.610 dB | 峰值目标降低 |
| 增强正则条件数 | `5.435e7` | `5.481e5` | 19.964 dB | 条件数降低 |
| 多功率最差标签NMSE | -55.362 dB | -57.121 dB | 1.759 dB | 最差值降低 |
| 多功率独立帧最差ACLR | 33.026863 dB | 33.019261 dB | 退化0.007601 dB | 退化不超过0.10 dB |

扩展结构、峰值加权和正则化不要求即时EVM都下降，因为三者分别优化标签表达、峰值误差和数值稳定。当前扩展结构额外改善0.728 dB独立帧EVM；长记忆支路超过扩展模型的交叉深度，所以增加有限深度后的射频收益仍然有限。多功率训练把最差标签NMSE改善1.759 dB，跨功率最差EVM改善0.521 dB，但独立帧最差ACLR下降0.007601 dB，且12 dBm EVM相对单功率正则模型退化0.385 dB；因此它是可量化的折中，不应描述为全面改善。源Wi-Fi波形的ACLR本底约为33 dB，ACLR验收采用“相对单功率正则模型退化不超过0.10 dB”的护栏；本次结果通过只表示没有明显频谱退化，不表示ACLR得到提升。
