...
`inc/lib/Channel.py` 描述PA前多通道耦合、独立非线性PA、PA后耦合、公共传播时延、外部仪表采样和板载反馈接收机。公开入口 `Channel.Process(...)` 固定返回二元组 `chOut, fbOut`：`chOut` 始终是前向主路测量；`sampleMode="forward"` 时 `fbOut` 是 `chOut` 的数值相同副本，`sampleMode="fb"` 时 `fbOut` 才是经过完整反馈接收链的DPD/ILC训练观测。两种模式都只运行一次PA、使用同一段记忆状态和同一个热周期，并在PA后耦合之后共享同一个 `channelDly`。
...
- `chOut` 用校准仪表直接观测PA主路输出。公共 `channelDly` 在forward/fb分路之前生效；所有 `fb...` 参数都不会进入 `chOut` 分支，因此不会把板载反馈接收机失真混入最终EVM、SNR、ACLR、IRR和功率，但Tx I/Q不平衡与公共传播时延仍然存在。
...
- 必须区分两种“校准”：PA功率设定闭环 `PowerCalibration` 仍观察PA后耦合、`channelDly` 和所有接收非理想之前的干净物理PA输出，`outputPowerDbm` 绝不是延迟后 `chOut` 或原始 `fbOut` 的表观功率；DPD/ILC校准或训练则使用 `fbOut` 做同步、MSE和系数更新。最终RF验收只把同一轮的 `chOut` 交给 `Analysis`。
...
`channelDly` 是PA后耦合节点到两个接收分支的公共传播时延，单位为复基带sample，默认 `0.0`，只接受非负有限实数。以下用 $D_{\mathrm{total}}$ 表示该参数值，代码把它分解为：
...
`fb...` 配置不会进入该公式；`channelDly` 不是FB专用参数，所以会进入该公式。即使兼容参数 `sampleMode="fb"`，公开 `Process` 返回的第一个数组仍遵循这个主路方程。同长零边界意味着有限记录开头出现延迟空白、尾部被截断；测量EVM、IRR或频响时应保留足够保护样点，并让同步搜索窗口覆盖总时延。
...
7. 仅在每个PA自身输出、PA后耦合之前测量有效区功率；公共 `channelDly`、公共移相、forward/fb接收链和白噪声都不进入校准误差。
...
本节参数描述公共 `channelDly` 之后的FB接收机专用同步误差，不应重复填写PA输出到两路采样器共有的传播时间。分数时延与采样频偏使用保持长度的插值模型。正的 `fbFractionalDelaySamples=d` 取源位置 $n-d$；采样偏差为 $\epsilon_{\mathrm{sfo}}=\mathrm{ppm}\times10^{-6}$：
...
- 主信号从左向右依次经过公开边界解码、已提交的逐链模拟drive、Tx I/Q调制器、PA前耦合、逐路PA、PA后耦合、公共 `channelDly` 和公共固定移相。功率闭环负责寻找并提交drive，不是一个PA输出后的缩放模块；其干净PA输出参考面不包含PA后耦合或公共传播时延。
- Tx I/Q位于PA之前，因此启用时forward与fb都包含它；FB I/Q位于采样分支内部，启用时只改变fb观测。两个enabled开关分别旁路自己的完整标量/FIR/DC模块，互不联动。
- 公开 `Process` 总是先构造 `chOut`，它在公共传播时延与公共移相后进入仪表前向支路，不经过任何 `fb...` 参数。`sampleMode="forward"` 把这个结果直接复制为 `fbOut`；`sampleMode="fb"` 让第二项共享 `channelDly` 后，再经过反馈增益/FIR、非线性/限幅、FB专用时延/CFO/SFO、I/Q不平衡/DC和反馈ADC。
...
| `Process(inputSignal, outputPowerDbm=None)` | 原始公开波形；可选共同目标dBm或逐链序列 | 返回 `(chOut, fbOut)`；默认稳态热模式每次都校准后只提交一个稳态周期。两路都含公共 `channelDly`；`forward` 复制第一项，`fb` 再执行完整反馈链；首次必须给目标，后续 `None` 复用最近成功目标 |
...
| `ProcessPaOutput(paOutputSignal)` | 已有各PA自身输出 | 兼容单输出入口；不运行PA或功率闭环，执行PA后耦合、公共 `channelDly` 及 `sampleMode` 所选采样路径 |
...
这里 `channelDly=3.25` 在160 MS/s下表示20.3125 ns公共传播：两个输出都包含它。`fbOut` 还额外包含 `12.18 sample` 的FB专用时延；同步器因而会观察到约 `15.43 sample` 的显式总时延，FIR群时延另计。`chOut` 用于最终EVM、SNR、ACLR、IRR和功率判断；显式 `sampleMode="fb"` 后得到的 `fbOut` 用于训练或验证反馈校准。不能只用未经校准的 `fbOut` 同时训练和评价DPD，否则容易得到“板载反馈EVM很好但主路仪表EVM变差”的虚假结论。
...
forward模式会完整跳过所有 `fb...` 非理想，但公共 `channelDly`、`phaseDegrees` 和三种互斥噪声控制仍然生效。对公开 `Process` 而言，这些公共效果只计算一次，随后复制，所以 `np.array_equal(chOut, fbOut)` 为True。
